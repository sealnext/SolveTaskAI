...
from app.schema.project import Project, ProjectCreate
...
class ProjectService:
	...
	async def _handle_existing_project(
		...
		logger.info(f'Linking user {user_id} to existing project {existing_project.id}.')
		...
		final_project_db = await self.project_repository.get_project_by_id_with_relations(
	...
	async def _create_new_project(self, project_data: ProjectCreate, user_id: int) -> ProjectDB:
		...
		new_project_db = await self.project_repository._add_project_db(project_data=db_project_data)
		...
		if not final_project_db:
			logger.error(f'Failed to retrieve newly created project {new_project_db.id}.')
			raise HTTPException(
				status.HTTP_500_INTERNAL_SERVER_ERROR,
		...
		try:
			...
			if 'unique constraint' in error_str:
				...
				if 'api_keys' in error_str:
					error_detail = f"Invalid API Key ID '{project_data.api_key_id}' provided."
			...
			logger.error(f'Database integrity error during project save: {e}', exc_info=True)
			...
			raise HTTPException(
				status.HTTP_500_INTERNAL_SERVER_ERROR,
				detail='An unexpected error occurred during project saving.',
			) from e
	...
	async def get_project_by_external_id(self, external_project_id: int) -> Project | None:
		project_db = await self.project_repository.get_by_external_id(external_project_id)
		if not project_db:
			return None
		return Project.model_validate(project_db)

	async def delete_project_by_external_id(self, user_id: int, external_project_id: int) -> bool:
		"""
