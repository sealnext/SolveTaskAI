...
class JiraClient(BaseTicketingClient):
    ...
    def _validate_project_key(self, project_key: str) -> None:
        ...
        if not project_key or not isinstance(project_key, str):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid project key"
            )
        ...
        while True:
            ...
            try:
                ...
                if isinstance(data, list):
                    projects = [
                        ExternalProject.model_validate(project) for project in data
                    ]
                ...
                if not projects and start_at == 0 and not all_projects:
                    ...
                    logger.warning(
                        "Initial project fetch returned empty list. Check permissions."
                    )
                ...
                logger.error(
                    f"Unexpected error fetching projects: {str(e)}", exc_info=True
                )
                raise HTTPException(
    ...
    async def get_tickets(self) -> AsyncGenerator[JiraIssueSchema, None]:
        ...
        try:
            ...
            total_tickets = (
                data_total.get("total", 0) if isinstance(data_total, dict) else 0
            )
            logger.info(
                f"Total tickets found for project {project_key}: {total_tickets}"
            )

        except Exception as e:
            logger.error(
                f"Failed to get total ticket count for project {project_key}: {e}"
            )
            raise HTTPException(
                status_code=500, detail=f"Failed to get ticket count: {e}"
            )
        ...
        for future in asyncio.as_completed(tasks):
            try:
                ...
                logger.error(
                    f"Error processing a ticket batch for project {project_key}: {e}"
                )
    ...
    async def get_ticket(self, ticket_id: str) -> JiraIssueContentSchema:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                raise HTTPException(
                    status_code=401, detail="Jira authentication failed."
                )
            elif e.response.status_code == 403:
                raise HTTPException(
                    status_code=403, detail="Permission denied to view ticket."
                )
            else:
                raise HTTPException(
                    status_code=e.response.status_code,
                    detail=f"Failed to get ticket: {e.response.text}",
                )
        except Exception as e:
            logger.error(f"Unexpected error getting ticket {ticket_id}: {e}")
            raise HTTPException(
                status_code=500, detail=f"Unexpected error getting ticket: {e}"
            )
    ...
    async def delete_ticket(self, ticket_id: str, delete_subtasks: bool = False) -> str:
        ...
        try:
            ...
            if status_code == 400:
                ...
                user_message = (
                    "Jira authentication failed. Please check API credentials."
                )
    ...
    async def get_ticket_edit_issue_metadata(self, ticket_id: str) -> dict:
        ...
        try:
            ...
            if status_code == 404:
                ...
                detail = (
                    f"Permission denied to view edit metadata for ticket {ticket_id}."
                )
            elif status_code == 401:
            ...
            logger.error(
                f"Unexpected error fetching edit metadata for {ticket_id}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_ticket_fields(
        self, ticket_id: str, fields: List[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching fields for {ticket_id} (Status {status_code}): {detail}"
            )
            raise HTTPException(status_code=status_code, detail=detail)
        except Exception as e:
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(response, dict):
                    logger.error(
                        f"Unexpected response type for boards: {type(response)}"
                    )
                ...
                status_code = e.response.status_code
                detail = f"Failed to fetch project boards for {project_key_or_id}: {e.response.text}"
                if status_code == 404:
                ...
                logger.error(
                    f"Error fetching project boards (Status {status_code}): {detail}"
                )
                raise HTTPException(status_code=status_code, detail=detail)
            except Exception as e:
                logger.error(
    ...
    async def get_board_sprints(
        ...
        while True:
            ...
            try:
                ...
                if not isinstance(response, dict):
                    logger.error(
                        f"Unexpected response type for sprints: {type(response)}"
                    )
                    break
                ...
                status_code = e.response.status_code
                detail = (
                    f"Failed to fetch sprints for board {board_id}: {e.response.text}"
                )
                if status_code == 404:
                    detail = f"Board {board_id} not found."
                elif status_code == 403:
                    detail = (
                        f"Permission denied to access sprints for board {board_id}."
                    )
                elif status_code == 401:
                    detail = "Jira authentication failed."

                logger.error(
                    f"Error fetching board sprints (Status {status_code}): {detail}"
                )
                raise HTTPException(status_code=status_code, detail=detail)
            except Exception as e:
                logger.error(
                    f"Unexpected error fetching sprints for board {board_id}: {str(e)}"
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def find_sprint_by_name(self, sprint_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not boards:
                logger.warning(
                    f"No boards found for project {project_key} to search for sprints."
                )
                return []

            matching_sprints = []
            sprint_fetch_tasks = [
                self.get_board_sprints(board["id"]) for board in boards
            ]

            # Gather sprints from all boards concurrently
            board_sprint_results = await asyncio.gather(
                *sprint_fetch_tasks, return_exceptions=True
            )
    ...
    async def search_issue_by_name(
        ...
        project_key = self.project.key
        logger.info(
            f"Searching for issue matching '{issue_name}' in project {project_key}"
        )
        ...
        try:
            ...
            status_code = e.response.status_code
            detail = (
                f"Failed to search issues in project {project_key}: {e.response.text}"
            )
            if status_code == 400:
                ...
                try:
                    ...
                    messages = error_json.get("errorMessages", [])
                    if messages:
                        detail = f"Invalid JQL query for project {project_key}: {'. '.join(messages)}"
                except Exception:
                    pass
            ...
            logger.error(
                f"Unexpected error searching issues in project {project_key}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def update_ticket(self, ticket_id: str, payload: Dict[str, Any]) -> str:
        ...
        try:
            ...
            user_message = (
                f"Failed to update ticket {ticket_id}: {error_detail}"  # Default
            )
    ...
    async def get_issue_createmeta(
        ...
        if not issue_type_name:
            raise HTTPException(
                status_code=400, detail="Issue type name is required for createmeta."
            )
        ...
        try:
            ...
            if isinstance(metadata, dict) and metadata.get("projects"):
                project_meta = metadata["projects"][
                    0
                ]  # Assume first project is the one requested
            ...
            status_code = e.response.status_code
            detail = f"Failed to fetch createmeta for {project_key}/{issue_type_name}: {e.response.text}"
            if status_code == 400:
                ...
                detail = (
                    f"Permission denied to fetch createmeta for project {project_key}."
                )
    ...
    async def create_ticket(self, payload: dict) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Error creating ticket (Status {e.response.status_code}): {error_msg}"
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Unexpected error creating ticket: {str(e)}"
            )
    ...
    async def get_issue_types(
        ...
        try:
            ...
            status_code = e.response.status_code
            project_context = (
                f" for project {target_project_id}" if target_project_id else ""
            )
            ...
            logger.error(
                f"Unexpected error fetching issue types{project_context}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
