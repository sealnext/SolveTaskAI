...
class JiraClient(BaseTicketingClient):
    ...
    def _build_url(self, *path_segments: str) -> str:
        ...
        path = f"rest/api/{self.API_VERSION}/" + "/".join(
            str(segment) for segment in path_segments
        )
                ...
                JiraIssueSchema.model_validate(
                    {
                        **issue,
                        "project_id": str(
                            issue.get("fields", {}).get("project", {}).get("id")
                        ),
                    }
                )
                for issue in issues
    ...
    async def get_tickets(self) -> AsyncGenerator[JiraIssueSchema, None]:
        ...
        # Process in batches
        for start_at in range(
            0, total_tickets, self.BATCH_SIZE * JIRA_MAX_CONCURRENT_REQUESTS
        ):
            ...
            for offset in range(
                0,
                min(
                    self.BATCH_SIZE * JIRA_MAX_CONCURRENT_REQUESTS,
                    total_tickets - start_at,
                ),
                self.BATCH_SIZE,
            ):
    ...
    async def delete_ticket(self, ticket_id: str, delete_subtasks: bool = False) -> str:
        ...
        try:
            ...
            error_msg = {
                400: "Please return and tell the user that you cannot delete issue with subtasks. Set deleteSubtasks=true to delete with subtasks: "
                + e.response.text,
                403: "Please return and tell the user that you do not have permission to delete issues. Check if you have 'Browse projects' and 'Delete issues' permissions: "
                + e.response.text,
                404: f"Please return and tell the user that the ticket {ticket_id} does not exist and he should check the ticket ID",
                401: "Please return and tell the user that your API credentials are invalid. Please check your API credentials: "
                + e.response.text,
    ...
    async def get_ticket_edit_issue_metadata(self, ticket_id: str) -> dict:
        ...
        try:
            response = await self.http_client.get(
                url, headers=self._get_auth_headers(), timeout=30.0
            )
    ...
    async def get_ticket_fields(
        ...
        return {field: response.get("fields", {}).get(field) for field in fields}
        ...
        try:
            ...
            for board in boards:
                ...
                for sprint in sprints:
                    matching_sprints.append({**sprint, "board_name": board["name"]})

            return matching_sprints

        except Exception as e:
            logger.error(
                f"Error searching for sprint '{sprint_name}' in project {self.project.key if hasattr(self.project, 'key') else 'unknown'}: {str(e)}"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def search_issue_by_name(
        self, issue_name: str, max_results: int = 5
    ) -> Dict[str, Any]:
    ...
    async def update_ticket(
        ...
        jira_payload = {
            ...
            "historyMetadata": {
                ...
                "actor": {"type": "application", "displayName": "AI Ticket Assistant"},
            },
        }
    ...
    async def revert_ticket_changes(
        ...
        try:
            ...
            history = await self._make_request(
                "GET", history_url, headers=self._get_auth_headers()
            )
            ...
            if version_number is not None:
                target_version = next(
                    (
                        ...
                        if v.get("id") == str(version_number)
                    ),
                    None,
                )
            ...
            for item in target_version.get("items", []):
                ...
                if field:
                    if field in ["summary", "description", "environment"]:
                        ...
                        if from_string is not None:
                            revert_payload["update"][field] = [
                                {"set": from_string.split(",")}
                            ]
                    else:
            ...
            if response.status_code == 204:
                logger.info(
                    f"Successfully reverted ticket {ticket_id} to version {target_version.get('id')}"
                )
                return
    ...
    async def get_issue_createmeta(self, project_key: str, issue_type: str) -> dict:
        ...
        try:
            ...
            if not response.get("projects") or not response["projects"][0].get(
                "issuetypes"
            ):
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
    ...
    def _parse_create_errors(self, e: httpx.HTTPStatusError) -> str:
        ...
        return error_map.get(
            e.response.status_code, f"Unknown error: {e.response.text}"
        )
