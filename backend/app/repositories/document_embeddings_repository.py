...
class DocumentEmbeddingsRepository:
    ...
    async def add_embeddings(
        ...
        if doc_count == 0:
            ...
            raise ValueError(
                "No documents to process. The project might be empty or inaccessible."
            )
        ...
        async with self._get_vector_store(unique_identifier) as vector_store:
            ...
            async def process_batch_with_stats(batch):
                ...
                try:
                    ...
                    elapsed_total = max(
                        current_time - process_start, 0.001
                    )  # Minimum 1ms
                    ...
                    if elapsed_since_last >= 1.0:  # Log every second
                        progress = (
                            (processed_count / doc_count) * 100 if doc_count > 0 else 0
                        )
            ...
            for i in range(0, len(batches), concurrent_batches):
                ...
                await asyncio.gather(
                    *(process_batch_with_stats(batch) for batch in batch_group)
                )
    ...
    async def _process_batch(
        ...
        try:
            ...
            prep_time = max(
                asyncio.get_running_loop().time() - prep_start, 0.001
            )  # Minimum 1ms
            ...
            embed_time = max(
                asyncio.get_running_loop().time() - embed_start, 0.001
            )  # Minimum 1ms
            ...
            db_time = max(
                asyncio.get_running_loop().time() - db_start, 0.001
            )  # Minimum 1ms
