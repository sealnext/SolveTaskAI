...
class RAGState(BaseModel):
    ...
    retry_retrieve_count: int = Field(
        0, description="Number of document retrieval attempts"
    )
...
async def retrieve_documents(state: RAGState, client: BaseTicketingClient) -> RAGState:
    ...
    try:
        ...
        documents_with_scores = (
            await vector_store.asimilarity_search_with_score_by_vector(
                query_embedding,
                k=k,
            )
        )
...
async def retry_retrieve_documents(
    state: RAGState, client: BaseTicketingClient
) -> RAGState:
    ...
    try:
        ...
        async def grade_single_document(doc: Document) -> tuple[Document, float]:
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Invalid grade response for {doc.metadata['key']}: {str(e)}"
                    )
                    return doc, 0.0

            except Exception as e:
                logger.error(
                    f"Failed to grade document {doc.metadata['key']}: {str(e)}"
                )
                return doc, 0.0
        ...
        try:
            grading_results = await asyncio.wait_for(
                asyncio.gather(
                    *[grade_single_document(doc) for doc in state.documents]
                ),
        ...
        for doc, score in grading_results:
            if score >= min_relevance_score:
                logger.debug(
                    f"Document {doc.metadata['key']} scored {score:.2f} (relevant)"
                )
                filtered_docs.append(doc)
            else:
                logger.debug(
                    f"Document {doc.metadata['key']} scored {score:.2f} (ignored)"
                )
                new_ignored.append(doc.metadata["key"])

        # Update state with size-limited ignore list
        state.documents = filtered_docs
        updated_ignore = list(set(state.ignore_tickets + new_ignored))[
            :max_ignore_tickets
        ]
        state.ignore_tickets = updated_ignore

        logger.info(
            f"Kept {len(filtered_docs)}/{len(state.documents)} relevant documents"
        )
        return state
    ...
    async def fetch_with_retry(ticket_id: str) -> Optional[str]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed for ticket {ticket_id}: {str(e)}"
                )
...
def create_rag_graph(
    ...
    builder.add_node(
        "retry_retrieval", partial(retry_retrieve_documents, client=client)
    )
