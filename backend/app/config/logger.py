...
logging.basicConfig(
    level=logging.DEBUG if DEBUG_MODE else logging.WARNING, handlers=[handler]
)
...
class GraphLogger:
    ...
    def log_tool_call(self, name: str, args: Any):
        ...
        self.logger.info(
            f"{Colors.WARNING}[TOOL-CALL]{Colors.ENDC} {_format_json(tool_info)}"
        )

    def log_state(self, state: Dict):
        """Log current state information"""
        self.logger.debug(
            f"{Colors.BLUE}[STATE]{Colors.ENDC} Current State:\n{_format_json(state)}"
        )
    ...
    def log_error(self, error: Exception, context: Optional[Dict] = None):
        ...
        self.logger.error(
            f"{Colors.FAIL}[ERROR]{Colors.ENDC} {_format_json(error_info)}"
        )
                ...
                if isinstance(value, dict):
                    ...
                    for subkey, subvalue in value.items():
                        if isinstance(subvalue, (int, float)):
                            current[key][subkey] = (
                                current[key].get(subkey, 0) + subvalue
                            )
    ...
    def log_metrics(self):
        ...
        if tokens:
            self.logger.debug(
                f"[METRICS] time: {self.metrics['execution_time']:.2f}s, tokens: {tokens.get('total_tokens', 0)} = {tokens.get('prompt_tokens', 0)}p + {tokens.get('completion_tokens', 0)}c"
            )
            ...
            for call in message.tool_calls:
                if isinstance(call, dict):
                    ...
                    if isinstance(args, dict):
                        ...
                        tool_calls.append(
                            f"{call.get('name', 'unknown')}({', '.join(args_str)})"
                        )
                    else:
...
def _format_tool_args(*args: Any, **kwargs: Any) -> str:
    ...
    formatted = {"positional_args": [], "keyword_args": {}}

    # Format positional arguments
    if args and len(args) > 0 and isinstance(args[0], dict):
        formatted["positional_args"] = [
            {k: v for k, v in args[0].items() if k != "config"}
        ]
    elif args:
            ...
            if func.__name__ == "call_model" and args and len(args) > 0:
                ...
                if isinstance(state, AgentState):
                    ...
                    if state.project_data:
                        logger.info(
                            f"[PROJECT] id: {state.project_data.get('id')}, name: {state.project_data.get('name')}"
                        )
                    ...
                    if state.messages:
                        ...
                        if isinstance(last_msg, (ToolMessage, HumanMessage)):
                            logger.info(
                                f"[INPUT] ({last_msg.__class__.__name__.lower()}): {last_msg.content}"
                            )
            ...
            try:
                ...
                if result is not None:
                                ...
                                if hasattr(msg, "response_metadata"):
                                    ...
                                    if metadata:
                                        ...
                                        if model or finish_reason:
                                            logger.info(
                                                f"[MODEL] name: {model}, finish_reason: {finish_reason}"
                                            )
                    ...
                    if tokens and tokens.get("total_tokens", 0) > 0:
                        logger.info(
                            f"[METRICS] time: {execution_time:.2f}s, tokens: {tokens['total_tokens']} = {tokens['prompt_tokens']}p + {tokens['completion_tokens']}c"
                        )
                    else:
