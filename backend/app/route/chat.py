...
from app.schema.chat import QuestionRequest, QuestionResponse
import logging
import uuid
...
async def get_chat_history(
    ...
    try:
        ...
        return {"messages": messages, "project_id": chat_session.project_id}
...
async def chat(
    ...
    project = await project_service.get_project_by_id(
        user_id, question_request.project_id
    )
    ...
    answer, chat_id = await agent.process_question(
        question_request.question, question_request.chat_id
    )
...
async def get_chat_history_list(
    ...
    try:
        ...
        for session in chat_sessions:
            message_count = (
                sum(1 for msg in session.messages if msg.get("role") in ["human", "ai"])
                if session.messages
                else 0
            )
            ...
            formatted_session = {
                ...
                "preview": session.messages[0].get("content", "")
