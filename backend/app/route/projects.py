...
from app.schema.project import ExternalProject, Project, ProjectCreate
...
async def delete_internal_project(
	...
	project: Project | None = await project_service.get_project_by_external_id(external_project_id)
	if project is None:
	...
	try:
		...
		raise HTTPException(
			status.HTTP_500_INTERNAL_SERVER_ERROR, 'Failed to delete project resources'
		)
