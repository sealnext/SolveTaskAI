...
router = APIRouter(
    prefix="/projects", tags=["projects"], dependencies=[Depends(auth_middleware)]
)


@router.post("/external")
async def get_all_external_projects(
    api_key: APIKey, ticketing_client: Callable = Depends(get_ticketing_client)
...
async def get_external_project_by_id(
    ...
    if not projects:
        raise HTTPException(
            status_code=404,
            detail="No projects found in external service. Check your API Key.",
        )
    return projects
    ...
    try:
        ...
        if not api_key:
            ...
            await project_service.delete_project_by_external_id(
                user_id, new_project.internal_id
            )
