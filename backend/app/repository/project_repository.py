...
from sqlalchemy import select, delete, and_, cast, String, insert, exists, func
...
class ProjectRepository:
    ...
    async def check_other_user_project_link(
    ...
    async def get_project_by_id_with_relations(
        self, project_id: int
    ) -> Optional[ProjectDB]:
        stmt = (
            select(ProjectDB)
    ...
    async def delete(self, user_id: int, project_id: int) -> bool:
        ...
        Relies on an outer transaction managed by get_db.
        """
        ...
        other_users_exist = await self.check_other_user_project_link(
            user_id, project_id
        )
    ...
    async def get_by_external_id(self, external_id: int) -> Optional[ProjectDB]:
        stmt = select(ProjectDB).where(ProjectDB.external_id == str(external_id))
        result = await self.db_session.execute(stmt)
        return result.scalar_one_or_none()

    async def is_project_associated(self, project_id: int) -> bool:
        query = select(
            exists().where(user_project_association.c.project_id == project_id)
        )
        result = await self.db_session.execute(query)
        return result.scalar()
