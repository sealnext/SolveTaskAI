...
class ChatSessionRepository:
	...
	async def get_by_id(self, chat_id: str) -> ChatSession | None:
		"""
	...
	async def add_messages(self, chat_id: str, new_messages: List[dict]) -> ChatSession:
		...
		if chat_session:
			...
			logger.debug(
				f'Added {len(new_messages)} messages to chat {chat_id}. Total messages: {len(chat_session.messages)}'
			)
		return chat_session
	...
	async def get_messages(self, chat_id: str) -> List[dict]:
		...
		if chat_session and chat_session.messages:
			...
			logger.debug(
				f'Retrieved {len(filtered_messages)} filtered messages from chat {chat_id}'
			)
