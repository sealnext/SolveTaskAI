...
from app.schemas.ticket import JiraIssueSchema
from app.schemas.project import Project
from fastapi import HTTPException, status
from pydantic import BaseModel, ValidationError
from app.config.config import DEFAULT_REQUEST_TIMEOUT
...
class BaseTicketingClient(ABC):
        ...
        **kwargs,
        ...
        try:
            ...
            if not isinstance(data, (dict, list)):
                raise ValueError(
                    f"Expected JSON object or array in response, got {type(data)}"
                )
    ...
    async def revert_ticket_changes(
        self, ticket_id: str, version_number: Optional[int] = None
    ) -> None:
