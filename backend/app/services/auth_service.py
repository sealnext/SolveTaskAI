...
from app.utils.security import decode_next_auth_token, hash_password, verify_password
...
class AuthService:
    ...
    async def authenticate(
        self, password: str, user: UserDB, request: Request
    ) -> Tuple[str, str]:
    ...
    def verify_and_decode_token(self, token: str, current_location: str) -> str:
        try:
            ...
            payload = jwt.decode(
                actual_token, JWT_SECRET_KEY, algorithms=[JWT_ALGORITHM]
            )
    ...
    def refresh_token_pair(
        self, expired_refresh_token: str, request: Request
    ) -> Tuple[str, str]:
        ...
        try:
            payload = jwt.decode(
                expired_refresh_token, JWT_SECRET_KEY, algorithms=[JWT_ALGORITHM]
            )
    ...
    def _clean_expired_tokens(cls):
        ...
        cls.revoked_tokens = {
            jti: exp for jti, exp in cls.revoked_tokens.items() if exp > current_time
        }
    ...
    def clear_session(response: JSONResponse) -> None:
        response.delete_cookie(
            key="next-auth.session-token", secure=True, httponly=True, samesite="lax"
        )
