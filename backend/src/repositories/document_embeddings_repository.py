...
from config import (
    ...
    OPENAI_MAX_BATCH_SIZE, OPENAI_REQUESTS_PER_MINUTE,
    OPENAI_TIMEOUT_SECONDS, DB_BATCH_SIZE,
...
class DocumentEmbeddingsRepository:
    ...
    async def add_embeddings(self, 
                           ...
                           total_documents: Optional[int] = None) -> None:
        ...
        async with self._get_vector_store(unique_identifier) as vector_store:
            ...
            async def process_batch_with_stats(batch):
                nonlocal processed_count, failed_count, total_embedding_time, total_db_time, last_progress_time
                try:
                    ...
                    elapsed_since_last = max(current_time - last_progress_time, 0.001)  # Minimum 1ms
    ...
    async def _process_batch(self, vector_store: PGVector, documents: List[DocumentEmbedding]) -> Dict[str, float]:
        """
