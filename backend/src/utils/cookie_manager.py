...
def set_auth_cookies(
    ...
    access_token_settings = CookieSettings(
        key="access_token",
        value=access_token,
        max_age=int(timedelta(minutes=access_token_expires or JWT_ACCESS_TOKEN_EXPIRE_MINUTES).total_seconds())
    )

    refresh_token_settings = CookieSettings(
        key="refresh_token",
        value=refresh_token,
        max_age=int(timedelta(days=refresh_token_expires or JWT_REFRESH_TOKEN_EXPIRE_DAYS).total_seconds())
    )
