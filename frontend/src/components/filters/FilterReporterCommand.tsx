...
import { UserSquare2, ChevronLeft, Plus, X } from "lucide-react"
import { CommandList } from "cmdk"
import { cn } from "@/lib/utils"
import { BaseFilterCommandProps } from "./types"

export function FilterReporterCommand({ 
  activeFilters, 
  ...
  // Filter only reporter filters
  const reporterFilters = React.useMemo(() => {
    return activeFilters.filter(filter => filter.icon === UserSquare2)
  }, [activeFilters])
  ...
  const handleAddReporter = () => {
    ...
    if (!trimmedName) {
      setError("Reporter name cannot be empty")
      return
    }

    if (reporterFilters.some(f => f.id.toLowerCase() === trimmedName.toLowerCase())) {
      setError("This reporter is already added")
      return
    }
      ...
      <div className="flex-none w-full px-4 py-3 border-b border-muted/20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-base font-medium leading-none mb-1">Reporter</h4>
            <p className="text-xs text-muted-foreground/70">Add reporter filters</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-3">
          {reporterFilters.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-2">
              ...
              <p className="text-sm text-muted-foreground/70">No reporter filters</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-2">
              {reporterFilters.map((filter) => (
                <button
                  key={filter.id}
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    <UserSquare2 className="h-3.5 w-3.5 text-primary/70" />
                  </div>
