...
export function FilterResolvedCommand({ 
  activeFilters, 
  ...
  // Filter only resolved date filters
  const resolvedFilters = React.useMemo(() => {
    return activeFilters.filter(filter => 
      filter.icon === CalendarIcon && 
      filter.metadata?.type === 'resolved'
    )
  }, [activeFilters])
  ...
  const handleAddPredefinedRange = (range: typeof DATE_RANGES[number]) => {
    ...
    if (resolvedFilters.some(f => f.id === `resolved_${range.id}`)) {
      onActiveFiltersChange(activeFilters.filter(f => f.id !== `resolved_${range.id}`))
      return
    }
  ...
  const handleDateSelect = React.useCallback((selectedDate: Date | undefined) => {
    ...
    const id = `resolved_custom_${selectedDate.toISOString()}`
    
    // Remove if already exists
    if (resolvedFilters.some(f => f.id === id)) {
    ...
    onActiveFiltersChange([...activeFilters, newFilter])
    setIsOpen(false)
  }, [activeFilters, resolvedFilters, onActiveFiltersChange])
  ...
  const handleDateRangeSelect = React.useCallback((range: DateRange | undefined) => {
    ...
    const id = `resolved_range_${range.from.toISOString()}_${range.to.toISOString()}`
      ...
      <div className="flex-none w-full px-3 py-2 border-b border-muted/20">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-medium leading-none">Resolved</h4>
            <p className="text-xs text-muted-foreground/70 mt-0.5">Filter by resolution date</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-2 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-4 p-1">
            {DATE_RANGES.map((range) => {
              const isSelected = resolvedFilters.some(f => f.id === `resolved_${range.id}`)
              return (
                <button
                  key={range.id}
