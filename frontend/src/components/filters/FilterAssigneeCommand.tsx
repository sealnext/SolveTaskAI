...
import { User, ChevronLeft, Plus, X } from "lucide-react"
import { CommandList } from "cmdk"
import { cn } from "@/lib/utils"
import { BaseFilterCommandProps } from "./types"

export function FilterAssigneeCommand({ 
  activeFilters, 
  ...
  // Filter only assignee filters
  const assigneeFilters = React.useMemo(() => {
    return activeFilters.filter(filter => filter.icon === User)
  }, [activeFilters])
  ...
  const handleAddAssignee = () => {
    ...
    if (!trimmedName) {
      setError("Assignee name cannot be empty")
      return
    }

    if (assigneeFilters.some(f => f.id.toLowerCase() === trimmedName.toLowerCase())) {
      setError("This assignee is already added")
      return
    }
      ...
      <div className="flex-none w-full px-4 py-3 border-b border-muted/20">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-base font-medium leading-none mb-1">Assignee</h4>
            <p className="text-xs text-muted-foreground/70">Add assignee filters</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-3">
          {assigneeFilters.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-2">
              ...
              <p className="text-sm text-muted-foreground/70">No assignee filters</p>
            </div>
          ) : (
